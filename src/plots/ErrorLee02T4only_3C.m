function opt = ErrorLee02T4only_3C(pe_All,peYS,FDname,ffs,cSave2,yLa1,yLa2,CS)
...
% legend
e_legend1 = legend({'Control','1 M','10 M'},'FontSize',ffs-8);
% % legend
...
%     'Import-aborted(T)','Import-transmitted(T)','Export-aborted(T)','Export-transmitted(T)'};
save4AllTable.Properties.RowNames = {'Control','0.1 M','1 M','10 M'};
%%%%%%%%%%%%%%%%%
...
if ~isempty(e_colors)
    for ii = 1:min(numel(hb), size(e_colors,1))
        hb(ii).FaceColor = 'flat';
        hb(ii).CData = repmat(e_colors(ii,:), size(hb(ii).CData,1), 1);
    end
end
for ii = 1:numel(hb)
...
eError01V_2ER = [eeE2_1;eeE2_2;eeE3_3;eeE2_4];
save3All = [eError01V_1IN;eError01V_2ER;];
save4AllTable = table(save3All(:,1),save3All(:,2),save3All(:,3),...
    'VariableNames',{'S1_F','S2_N','S3_B'});
save4AllTable.Properties.RowNames = {'Control','0.1 M','1 M','10 M','Control STD','0.1 M STD','1 M STD','10 M STD'};
%%%%%%%%%%%%%%%%%
writetable(save4AllTable,F6name,'Delimiter','bar','WriteRowNames',true);
F6nameX = [F6name '.xlsx'];
writetable(save4AllTable,F6nameX,'WriteRowNames',true,'Sheet',1);
...
save4AllTable = table(save3All(:,1),save3All(:,2),save3All(:,3),...
    'VariableNames',{'S1_F','S2_N','S3_B'});
save4AllTable.Properties.RowNames = {'Control','0.1 M','1 M','10 M','Control SE','0.1 M SE','1 M SE','10 M SE'};
%%%%%%%%%%%%%%%%%
writetable(save4AllTable,F6name,'Delimiter','bar','WriteRowNames',true);
F6nameX = [F6name '.xlsx'];
writetable(save4AllTable,F6nameX,'WriteRowNames',true,'Sheet',1);
