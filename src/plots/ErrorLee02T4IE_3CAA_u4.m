function opt = ErrorLee02T4IE_3CAA_u4(pe_All,peYS,FDname,ffs,cSave2,yLa1,yLa2,CS,AA,AB)
...
e_colorsBar = [bla1;blu1;mag1;cya1];
...
% colour spot
for DD=AA
     plot(eeP2_1(DD,:),'-','Marker','o','MarkerFaceColor',e_colorsT1(1, :),...
         'Color',e_colorsT1(1,:),'LineWidth',3);
     hold on;
end
...
% end
for DD=AA
     plot(eeP2_3(DD,:),'-','Marker','o','MarkerFaceColor',e_colorsT2(1, :),...
         'Color',e_colorsT2(1,:),'LineWidth',3);
     hold on;
end
for DD=AA
     plot(eeP2_4(DD,:),'-','Marker','o','MarkerFaceColor',e_colorsT3(1, :),...
         'Color',e_colorsT3(1,:),'LineWidth',3);
     hold on;
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Save txt values
save3All = [eeP2_1;eeP2_3;eeP2_4];
save4AllTable = table(save3All(:,1),save3All(:,2),save3All(:,3),save3All(:,4),...
    'VariableNames',{'S1_F','S2_N','S3_B','S123_NPC'});
% save4AllTable.Properties.RowNames = {'Import-aborted(C)','Import-transmitted(C)','Export-aborted(C)','Export-transmitted(C)',...
%     'Import-aborted(T)','Import-transmitted(T)','Export-aborted(T)','Export-transmitted(T)'};
save4AllTable.Properties.RowNames = {'Importing(C)','Exporting(C)',...
    'Importing(1M)','Exporting(1M)',...
    'Importing(10M)','Exporting(10M)'};
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Save txt values
save3All = [(eeP2_1-YT12(1,1))./(YT12(1,2)-YT12(1,1));...
...
% Error bar
eError01V_1IN = [eeP2_1(AA,:);eeP2_3(AA,:);eeP2_4(AA,:)];
eError01V_2ER = [eeE2_1(AA,:);eeE2_3(AA,:);eeE2_4(AA,:)];
...
set(gca,'FontSize',ffs+4,'XTicklabels',{'S1(F)','S2(N)','S3(B)','NPC'});
% set(gca,'XTick',1:1:3,'FontSize',ffs-7,'XTicklabels',{xLa1,xLa2,xLa3})
...
save4AllTable.Properties.RowNames = {'Importing(C)','Exporting(C)',...
    'Importing(1M)','Exporting(1M)',...
    'Importing(10M)','Exporting(10M)',...
    'Importing(C) STD','Exporting(C) STD',...
    'Importing(1M) STD','Exporting(1M) STD',...
    'Importing(10M) STD','Exporting(10M) STD'};
%%%%%%%%%%%%%%%%%
writetable(save4AllTable,F6name,'Delimiter','bar','WriteRowNames',true);
F6nameX = [F6name '.xlsx'];
writetable(save4AllTable,F6nameX,'WriteRowNames',true,'Sheet',1);
...
% Error bar
eError01V_1IN = [eeP2_1(AA,:);eeP2_3(AA,:);eeP2_4(AA,:)];
eError01V_2ER = [eeE3_1(AA,:);eeE3_3(AA,:);eeE3_4(AA,:)];
% Grouped bar + errorbars (no 3rd-party helper)
Y = eError01V_1IN';
E = eError01V_2ER';
hb = bar(Y,'grouped');
...
save4AllTable.Properties.RowNames =  {'Importing(C)','Exporting(C)',...
    'Importing(1M)','Exporting(1M)',...
    'Importing(10M)','Exporting(10M)',...
    'Importing(C) SE','Exporting(C) SE',...
    'Importing(1M) SE','Exporting(1M) SE',...
    'Importing(10M) SE','Exporting(10M) SE'};
%%%%%%%%%%%%%%%%%
writetable(save4AllTable,F6name,'Delimiter','bar','WriteRowNames',true);
F6nameX = [F6name '.xlsx'];
writetable(save4AllTable,F6nameX,'WriteRowNames',true,'Sheet',1);
